 ...
 * @brief Demonstrates the use of an array of pointers to efficiently sort a
 * set of lines.
 *
 * @version 0.1
 * @date 2025-05-10
 ...
 * @return -1.
 */
int readlines(int maxlines, char* lineptr[]);
 ...
 */
void writelines(int nlines, char* lineptr[]);
 ...
 */
int main() {
    int nlines = readlines(MAXLINES, lineptr);
    if (nlines >= 0) {
        my_qsort(lineptr, 0, nlines - 1);
        writelines(nlines, lineptr);
        return EXIT_SUCCESS;
    } else {
        return EXIT_FAILURE;
    }
