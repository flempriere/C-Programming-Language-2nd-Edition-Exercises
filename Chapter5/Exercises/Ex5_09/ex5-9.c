 ...
 * @brief Solution to Exercise 5.9 from The C Programming Language, 2nd Edition.
 *
 * Rewrite the routines day_of_year and month_day with pointers instead of
 * indexing.
 *
 * @remark Keeps the error handling code from Exercise 5.8
 *
 * @version 0.1
 * @date 2025-05-10
 *
...
/**
 * @brief Array containing the number of days in the months for a non_leap year.
 *
 */
static char non_leap_days[] = { 0,  31, 28, 31, 30, 31, 30,
                                31, 31, 30, 31, 30, 31 };

/**
 * @brief Array containing the number of days in the months for a leap year.
 *
 */
static char leap_days[] = { 0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };

/**
 * @brief Static containing the number of days in
 * a month, with seperate subarrays for leap and
 * non-leap years.
 *
 */
static char* daytab[] = { non_leap_days, leap_days };
...
int day_of_year(int year, int month, int day) {
    ...
    char* p = *(daytab + IS_LEAP(year));
    if (day > *(p + month) || day < 1) { return 0; }
    while (--month) { day += *(++p); }
    return day;
}
...
enum truth test_calender_functions(int year, int month, int day, int expected) {
    ...
    if (month < JAN || month > DEC) { month = NONE; }
    if (day < 1 || day > daytab[IS_LEAP(year)][month]) { day = 0; }
    if (year < 0) {
        day = 0;
        month = 0;
    }
