 ...
 * @brief Solution to Exercise 5.15 from The C Programming Language, 2nd
 * Edition.
 *
 * Add the option -f to fold upper and lower case together, so that case
 * distinctions are not during sorting; for example, `a` and `A` compare equal.
 *
 * @version 0.1
 * @date 2025-05-16
 *
...
int main(int argc, char* argv[]) {
    static unsigned options = 0; /* variable containing the argument flags */
    while (--argc > 0 && (*++argv)[0] == '-') {
    ...
    if (nlines >= 0) {
        int (*cmp)(void*, void*) = CAST_COMPARISON_FUNCTION(strcmp);
        if (options & NUMERIC) {
            cmp = CAST_COMPARISON_FUNCTION(numcmp);
        } else if (options & FOLD) {
            cmp = CAST_COMPARISON_FUNCTION(foldcmp);
        }
        my_qsort((void**) lineptr, 0, nlines - 1, cmp);
